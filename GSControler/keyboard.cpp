...
{
  //  QMessageBox::information(this,"aaaaaa","2212");
...
//    if (!widget)
 ...
 //   QWidget *q = QApplication::focusWidget();
   ...
   // if (q != NULL)
 //   {
        e = dynamic_cast<QLineEdit *>(widget);//dynamic_cast用于判断q的类型是否为QLineEdit如果是，那么e就为QLineEdit。
        if (e != NULL)  Window_show.ShowKeyBoard(this);;

        t = dynamic_cast<QTextEdit *>(widget);
        if (t != NULL)  Window_show.ShowKeyBoard(this);;
  //  }
